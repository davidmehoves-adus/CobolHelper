...
3. **Parallel when possible.** If a request triggers multiple skills (e.g., "explain this program and find any bugs" = business-analyst + code-specialist), spawn them in parallel. **Exception:** the documentor is never spawned in parallel with analysis skills — see rule 5.
...
5. **Documentor is always sequenced last.** The documentor must never be spawned until it has content to document. When the user's request triggers both analysis skills and documentation, spawn the analysis skills first, wait for their results, deliver the chat response, then spawn the documentor with the analysis findings included in its prompt. The documentor builds on existing findings — it does not re-analyze source code from scratch.
6. **Simple questions don't need forked skills.** If the user asks something Claude can answer directly from knowledge files or a quick code read, just answer — don't over-orchestrate. But if the answer requires reading COBOL source files, always fork.
7. **Documentor review loop.** After the documentor returns its `.md` draft, the top-level agent must review it before the documentor generates `.docx`. The top-level agent reads the output file and checks it against these standards:
